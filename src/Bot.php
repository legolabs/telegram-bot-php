...
class Bot
{
	...
	{
		$parts = [['name' => 'url', 'contents' => $setWebhook->getUrl()]];

		if ($setWebhook->getCertificate())
		{
			$parts[] = ['name' => 'certificate', 'contents' => $setWebhook->getCertificate(), 'filename' => 'self-signed-cert.pem'];
		}

		return $this->httpClient->post($this->getMethodUrl('setWebhook'), ['multipart' => $parts]);
	}
	...
	{
		$response = $this->httpClient->post($this->getMethodUrl('sendMessage'), ['json' => get_values($sendMessage)]);
	...
	public function sendPhoto(SendPhoto $sendPhoto): ResponseInterface
	{
		...
		{
			return $this->httpClient->post($this->getMethodUrl('sendPhoto'), ['json' => get_values($sendPhoto)]);
		}
		$values = get_values($sendPhoto);

		$data[] = ['name' => 'photo', 'contents' => $values['photo'], 'filename' => 'picture.jpg'];
		unset($values['photo']);
		...
		return $this->httpClient->post($this->getMethodUrl('sendPhoto'), ['multipart' => $data]);
	}
	...
	public function sendDocument(SendDocument $sendDocument): Message
	{
		...
		if ($doc instanceof FileId || $doc instanceof FileUrl)
		{
			...
			$response = $this->httpClient->post($this->getMethodUrl('sendDocument'), ['json' => $data]);
		...
		{
			$data[] = ['name' => 'document', 'contents' => $doc->getContent(), 'filename' => $doc->getFileName()];
			...
			$response = $this->httpClient->post($this->getMethodUrl('sendDocument'), ['multipart' => $data]);
	...
	{
		return $this->httpClient->post($this->getMethodUrl('sendInvoice'), ['json' => get_values($sendInvoice)]);
	}

	public function answerCallbackQuery(AnswerCallbackQuery $answerCallbackQuery): ResponseInterface
	{
		return $this->httpClient->post($this->getMethodUrl('answerCallbackQuery'), ['json' => get_values($answerCallbackQuery)]);
	}

	public function answerPreCheckoutQuery(AnswerPreCheckoutQuery $answerPreCheckoutQuery): ResponseInterface
	{
		return $this->httpClient->post($this->getMethodUrl('answerPreCheckoutQuery'), ['json' => get_values($answerPreCheckoutQuery)]);
	}

	public function editMessageText(EditMessageText $editMessageText): ?Message
	{
		$response = $this->httpClient->post($this->getMethodUrl('editMessageText'), ['json' => get_values($editMessageText)]);
	...
	public function editMessageReplyMarkup(EditMessageReplyMarkup $editMessageReplyMarkup): ?Message
	{
		$response = $this->httpClient->post($this->getMethodUrl('editMessageReplyMarkup'), ['json' => get_values($editMessageReplyMarkup)]);
	...
	{
		$response = $this->httpClient->post($this->getMethodUrl('deleteMessage'), ['json' => get_values($deleteMessage)]);
	...
	{
		$response = $this->httpClient->post($this->getMethodUrl('getFile'), ['json' => get_values($getFile)]);
